...
survey_data <- read_csv("./../outputs/data/survey_data.csv")
...
# Model
In this project, we are interested in predicting the popular vote outcome of the USA federal election. To do this we built a multilevel regression model with a binomial family based on the 25th June, 2020 Nationscape Data set (survey_data). Then, we applied the model to the census_data which we obtained from the 2018 American Community Surveys to make the prediction. In the following sub-sections, we will introduce our model and the post-stratification strategies specifically. In addition, we will present the model justification procedures.
...
```{r, echo=F}
kable(survey_data %>% 
  summarise(support_trump=sum(vote_trump)/nrow(survey_data)), caption = "Table 1: Estimates of Supporting Trump")
```
...
```{r, echo=F}
kable(state_winner, caption = "Table 2: Winners for each American State")
```
...
```{r, echo=F}
kable(election_result, caption = "Table 3: Total Votes of Donald Trump and Joe Biden")
```
